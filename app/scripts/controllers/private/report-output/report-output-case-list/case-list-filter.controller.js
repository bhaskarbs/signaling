 ...
 */
'use strict';
angular.module('saintApp').controller('CaseListModalfilterController', ['$scope', 'ReportOutputFactory', 'ConstantService', function (scope, ReportOutputFactory, ConstantService) {
  ...
  scope.fnInit = function () {
    ...
    scope.caseListData = ReportOutputFactory.data;
    ...
    ReportOutputFactory.getConfig(ConstantService.FILTER_PARAM_FILTER).then(
      function (result) {
        if (result.error) {
  ...
  /**
   *
   * @param columnName KEY_NAME of the clicked category.
   * @param objectIndex Index of the object within the array of categories.
   * @description This Function used to inject the inside contents dynamically, It is called on cick of any filter category.
   */
  scope.fnClickCategory = function (filterName,columnName, index,filterType) {
    if (columnName && filterType!==ConstantService.FILTER_LIST_KEY_TEXT) {
      ...
      if (scope.categoryData[index].contents.length <= 0) {
        ReportOutputFactory.getFilterContent(scope.queryParams, scope.columnName).then(function (result) {
          if (result.error) {
          }
          else {
  ...
  scope.fnFilterClick = function (filter, categoryName, columnName, category, filterType,selectAll) {
    var existingFilter = _.findWhere(ReportOutputFactory.data.selectedFilters, {'filterName': categoryName});
    ...
    ReportOutputFactory.data.selectedFilters.push(newSelectedFilter);
      ...
      var objectIndex = ReportOutputFactory.data.selectedFilters.findIndex(function (existingFilter) {
        return existingFilter.filterName === categoryName;
      });
      if ((ReportOutputFactory.data.selectedFilters[objectIndex].contents.indexOf(filter) < 0) && (selectAll === true || selectAll === undefined)) {
      ReportOutputFactory.data.selectedFilters[objectIndex].contents.push(filter);
      } else if ((ReportOutputFactory.data.selectedFilters[objectIndex].contents.indexOf(filter) >= 0) && (selectAll === false || selectAll === undefined)) {
        var filterIndex = ReportOutputFactory.data.selectedFilters[objectIndex].contents.lastIndexOf(filter);
      ReportOutputFactory.data.selectedFilters[objectIndex].contents.splice(filterIndex, 1);
        if (ReportOutputFactory.data.selectedFilters[objectIndex].contents.length === 0) {
        ReportOutputFactory.data.selectedFilters.splice(objectIndex, 1);
        }
      }
    }
  ...
  scope.fnCheckbox = function (filter, categoryName) {
    ...
    if (!existingFilter) {
      ...
      if (ReportOutputFactory.data.selectedFilters[index].contents.indexOf(filter) < 0) {
        return false;
      } else {
        return true;
      }
  ...
  scope.fnSelectAllbox = function (selectAllCatgoryObj) {
    var existingFilterObj = _.findWhere(ReportOutputFactory.data.selectedFilters, {'filterName': selectAllCatgoryObj.filterName});
    ...
    if (!existingFilterObj) {
      ...
      var index = ReportOutputFactory.data.selectedFilters.findIndex(function (existingFilterObj) {
      ...
      if(index>=0){
        ...
        var selectedFilters=ReportOutputFactory.data.selectedFilters[index].contents;
